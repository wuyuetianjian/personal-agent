...
- Keep `configs/config.example.yaml` self-documenting: every example key must include concise Chinese and English comments that explain the setting, accepted value shape where useful, and privacy or safety impact when relevant. The `models.registry` example must include complete representative entries for local chat/planning, local embedding, local reranking, and disabled public remote chat models so operators can see every supported model shape. Comment-only updates must not change default runtime behavior.
- Serialize OpenAI-compatible chat messages with the protocol-required lowercase `role` and `content` JSON fields so configured local and public-compatible providers can accept reasoning, synthesis, planner, and chat requests.
- Route every user task through a shared `Runtime.DispatchTask` entry point. Dispatch must search local memory/vector/BM25 evidence first; sufficient evidence invokes only lightweight synthesis, while insufficient evidence invokes Leader planning and delegated Sub-Agents before synthesis.
- Wire configured OpenAI-compatible local providers into the Runtime model path so Leader planning and role-specific Sub-Agents use their selected models.
...
- Wire configured role-specific Sub-Agent model IDs into model-backed `reasoning.local` and `synthesis.local` execution so those nodes use `agent.subagents.reasoning.model_id` and `agent.subagents.synthesis.model_id` instead of always reusing the Leader provider.
...
- Soak tests verify `pachat release soak` produces a passing JSON report for the offline Runtime gate, records workflow, memory, goroutine, SQLite, notification, and external-artifact metrics, and can run repeatedly against the same regression database without fixed fixture id collisions.
