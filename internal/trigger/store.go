...
func (s Store) PutState(ctx context.Context, st State) error {
	...
	_, err := s.DB.ExecContext(ctx, `INSERT INTO trigger_state (trigger_id, last_fired_at, last_success_at, last_failure_at, next_fire_at, last_event_hash, consecutive_failures, cooldown_until, previous_state, current_state, last_transition_at, last_check_at, last_notification_at, state_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(trigger_id) DO UPDATE SET last_fired_at = excluded.last_fired_at, last_success_at = excluded.last_success_at, last_failure_at = excluded.last_failure_at, next_fire_at = excluded.next_fire_at, last_event_hash = excluded.last_event_hash, consecutive_failures = excluded.consecutive_failures, cooldown_until = excluded.cooldown_until, previous_state = excluded.previous_state, current_state = excluded.current_state, last_transition_at = excluded.last_transition_at, last_check_at = excluded.last_check_at, last_notification_at = excluded.last_notification_at, state_json = excluded.state_json`, st.TriggerID, st.LastFiredAt, st.LastSuccessAt, st.LastFailureAt, st.NextFireAt, st.LastEventHash, st.ConsecutiveFailures, st.CooldownUntil, st.PreviousState, st.CurrentState, st.LastTransitionAt, st.LastCheckAt, st.LastNotificationAt, st.StateJSON)
	return err
}

func (s Store) GetState(ctx context.Context, triggerID string) (State, error) {
	var st State
	err := s.DB.QueryRowContext(ctx, `SELECT trigger_id, last_fired_at, last_success_at, last_failure_at, next_fire_at, last_event_hash, consecutive_failures, cooldown_until, previous_state, current_state, last_transition_at, last_check_at, last_notification_at, state_json FROM trigger_state WHERE trigger_id = ?`, triggerID).Scan(&st.TriggerID, &st.LastFiredAt, &st.LastSuccessAt, &st.LastFailureAt, &st.NextFireAt, &st.LastEventHash, &st.ConsecutiveFailures, &st.CooldownUntil, &st.PreviousState, &st.CurrentState, &st.LastTransitionAt, &st.LastCheckAt, &st.LastNotificationAt, &st.StateJSON)
	return st, err
}
